...
| **What is a Bloom Filter?** |	A memory-efficient probabilistic data structure used by ScyllaDB to determine whether a key might exist or definitely does not exist in an SSTable. **Key Point:** Bloom Filters are used only during read operations, not writes.|
