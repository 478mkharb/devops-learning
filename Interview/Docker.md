...
**Definition:** Docker is a platform for building, packaging,
distributing, and running applications as containers.

Docker packages an application with the files and dependencies required
for its runtime environment into an image.
...
**Definition:** A container is an isolated process created from a
container image.

A Linux container normally shares the host Linux kernel while using
isolation mechanisms such as namespaces and resource controls such as
cgroups.
...
Containers normally do not boot a complete guest operating system. They
share the host kernel and package the application environment rather
than a full guest OS.

That usually means lower startup and memory overhead than a VM, although
containers still consume CPU, memory, storage, and network resources.
...
A build can produce a versioned container image that can be promoted
...
The exact implementation can vary by Docker release and platform, but
...
The Docker Client, commonly the `docker` CLI, is the user-facing
interface used to send requests to Docker Engine.
...
The Docker Engine API is the interface used by clients and tools to
request operations from Docker Engine.
...
On a typical Linux installation, the Docker daemon exposes a Unix socket
at:
...
The Docker CLI commonly uses this local socket to communicate with the
daemon.

**Security:** Access to the Docker daemon is highly privileged. Giving
an untrusted process access to the socket can provide extensive control
over the host.
...
**Definition:** Control groups are Linux kernel mechanisms for
accounting for and controlling resource usage by groups of processes.
...
`containerd` is a container runtime component responsible for container
lifecycle management and related image/runtime operations. Docker Engine
uses containerd as part of its runtime architecture.

------------------------------------------------------------------------

## Q17. What is runc?

`runc` is a low-level OCI-compliant container runtime implementation
used to create and run containers according to OCI runtime
specifications.
...
OCI publishes open standards for container image formats and container
runtimes.
...
This is one reason the application process and shutdown behavior matter
in containers.
...
**Definition:** A Docker image is an immutable, layered package
containing filesystem content and metadata used to create containers.
...
Base filesystem content
        +
Filesystem changes from build steps
        +
Application content
        =
...
Many filesystem-changing Dockerfile instructions contribute to image
layers. Metadata-only instructions do not necessarily create filesystem
layers, and modern BuildKit can optimize how build results and layers
...
Docker can reuse previously built results when the relevant instruction
...
A source-code change can then avoid invalidating the
...
Tags are mutable references; the same tag can later point to different
image content.
...
A digest is a content-addressed identifier for specific image content,
commonly an image manifest or other OCI content.
...
A container normally uses read-only image layers plus a writable
container layer.
...
Changes made by the container are stored through the writable container
layer rather than modifying the original image layers. The exact
implementation depends on the storage driver, but unchanged image data
can be shared rather than duplicated for every container.
...
**Definition:** A Dockerfile is a text file containing instructions used
...
**Definition:** `CMD` specifies the default command and/or default
arguments used when a container starts.
...
**Definition:** `ENTRYPOINT` specifies the primary executable that the
container is intended to run. Runtime arguments are appended to an
exec-form `ENTRYPOINT`; `--entrypoint` can be used to override the image
ENTRYPOINT.
...
Exec form is generally preferred for application processes because it
avoids an extra shell and usually provides cleaner signal handling.
...
`ADD` copies files/directories and also supports additional
Docker-specific behavior, including extraction of local tar archives.
...
`WORKDIR` sets the working directory for subsequent Dockerfile
instructions and the default working directory for the container.
...
`USER` specifies the user/group used by subsequent Dockerfile
instructions and by default for the container process.
...
`EXPOSE` documents the port that the application intends to listen on
...
`VOLUME` declares a mount point in the image for externalized/persistent
data. Persistence depends on the volume or mount used at runtime.

``` dockerfile
VOLUME ["/var/lib/myapp"]
```

In many deployments, volumes are explicitly managed at runtime or by an
orchestrator.
...
`HEALTHCHECK` defines a test Docker can use to determine whether a
...
A health status is different from merely knowing that the main process
exists.
...
`.dockerignore` excludes files/directories from the Docker build
context.
...
It can reduce build-context size and help prevent unnecessary or
sensitive files from being sent as build context.
...
A multistage Dockerfile uses multiple `FROM` stages so build
dependencies can be separated from the final runtime image.
...
use an attachable overlay when the Docker hosts participate in Swarm
mode.
...
It is a Docker bridge network created by the user for controlled
...
This reduces network isolation and should be used deliberately. Normal
`-p` host-port publishing is not used with host networking because the
...
For containers on a shared user-defined network, use the database
...
Use an appropriate secret-management solution for the build/deployment
platform.
...
This constrains CPU usage; it does not mean the container owns one
...
Restart policies control automatic restart behavior; they do not fix the
...
A common issue is an application listening only on `127.0.0.1` inside
the container when it needs to accept connections through the container
network.
...
and that the application is actually listening on the expected container
port/interface.
...
Remove unused resources selectively after confirming they are safe to
delete.
...
A generic application-level failure. The exact meaning depends on the
application.
...
Docker Compose is a declarative tool for defining and running
...
It can express dependency/startup ordering, but a dependency can be
started before its application is actually ready to accept requests.
...
It can be useful for experimentation, but Dockerfiles are generally
preferred for reproducible builds.
...
A compromised workload may therefore gain extensive control over
Docker-managed resources and potentially the host.
...
Also verify that the application is listening on the expected interface
and port.
...
If TCP fails, investigate network path, listening port, firewall, or
service state rather than only DNS.
...
The container can communicate with the host Docker daemon. If the CI
container is compromised, the attacker may obtain extensive control over
Docker-managed resources and potentially the host.

Use a security design appropriate to the CI architecture rather than
assuming the socket is harmless.
...
Docker is a container platform/ecosystem. Kubernetes is a container
...
Do not confuse BuildKit with the container runtime. BuildKit builds
images; it does not run application containers.

------------------------------------------------------------------------

## Q131. What is `docker buildx`?

`docker buildx` is the Docker CLI plugin/interface used to access
advanced BuildKit-based build features.
...
## Q132. What causes Docker build cache invalidation?

Docker can reuse previous build results when the relevant build inputs
have not changed. Cache invalidation can occur when an instruction, its
relevant inputs, or earlier build state changes.

For example, this is often less cache-efficient:
...
because a change to any source file can invalidate the
dependency-installation step.
...
Now application source changes can often reuse the
dependency-installation cache.

**Interview point:** Dockerfile instruction ordering affects both build
speed and cache reuse.

------------------------------------------------------------------------

## Q133. How do you build a Docker image for multiple CPU architectures?

Use BuildKit/buildx with the `--platform` option.
...
The registry can then contain a multi-platform image index that points
to architecture-specific image manifests.

**Interview point:** A multi-platform image is not one binary image that
runs identically on every CPU. It is a higher-level reference that
resolves to the appropriate platform-specific image.

------------------------------------------------------------------------

## Q134. How should secrets be handled when building or running containers?

Do not treat Dockerfile `ENV` or `ARG` as a general-purpose secret
store.

For build-time secrets, use a build mechanism designed for secrets, such
as BuildKit secret mounts, so the secret is not intentionally baked into
the final image.
...
## Q135. What does `--privileged` do, and why is it dangerous?

`--privileged` substantially relaxes the isolation and capability
restrictions normally applied to a container and gives the container
access to many host-level capabilities and devices.
...
It should not be used as a generic solution for permission problems.

Before using it, identify the exact capability or device requirement and
grant the minimum required access instead.

**Interview point:** A privileged container significantly weakens the
container security boundary.

------------------------------------------------------------------------

## Q136. What are Linux capabilities in Docker?

Linux capabilities divide traditional root privileges into smaller
units.

Docker can drop unnecessary capabilities or add a specific capability
when required.
...
The security principle is **least privilege**: give the container only
the capabilities it actually needs.

This is generally preferable to using `--privileged` when only a narrow
capability is required.

------------------------------------------------------------------------

## Q137. What are Docker logging drivers?

Docker logging drivers determine how container stdout/stderr logs are
...
In production, logs are often forwarded to a centralized logging system
...
## Q138. What is Rootless Docker?

Rootless Docker runs Docker components without requiring a root-owned
Docker daemon for normal operation.

The goal is to reduce the impact of a compromise by avoiding a daemon
that has full root privileges on the host.

It can improve security, but it also has feature, networking, storage,
and operational considerations that differ from a traditional rootful
Docker Engine setup.

**Interview point:** Rootless mode is a security model, not simply
another container image option.

------------------------------------------------------------------------

## Q139. Docker daemon is not running. How do you troubleshoot it?
...
Check whether the Docker CLI can communicate with the daemon:
...
restart and verify with `docker info`.
...
A normal container is an isolated process environment, not a complete
guest OS.
...
137 corresponds to a process terminated with SIGKILL; OOM is a common
cause.
...
## Trap 15: `--privileged` is a normal fix for permission errors

**Wrong.**

Identify the exact capability or device requirement and grant the
minimum required access. `--privileged` significantly weakens isolation.

------------------------------------------------------------------------

## Trap 16: BuildKit is the container runtime

**Wrong.**

BuildKit builds images. It is not the runtime that executes application
containers.

------------------------------------------------------------------------

## Trap 17: A multi-platform image is one image binary for every CPU

**Wrong.**

A multi-platform reference normally resolves to platform-specific image
manifests/images.

------------------------------------------------------------------------

## Trap 18: Docker `ARG`/`ENV` is a secure secret store

**Wrong.**

Use purpose-built secret handling for build-time and runtime
credentials.
