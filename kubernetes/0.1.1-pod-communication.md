...
### 4.1 Pod To Pod Communication on same node

Interview answer

When two Pods are running on the same node, the CNI plugin creates a virtual Ethernet pair, also called a veth pair, for each Pod. One end of the veth pair is placed inside the Pod network namespace as eth0, and the other end remains on the node. The node-side interfaces are connected through a Linux bridge such as cni0, or handled by another CNI datapath depending on the plugin. Therefore, traffic travels locally within the node and normally does not leave the node or require VXLAN encapsulation.
...
When two Pods are running on different nodes, the source Pod sends traffic through its Pod network interface to the source node. The CNI plugin then forwards the traffic toward the destination node using routing, encapsulation, or the cloud network, depending on the CNI implementation. The destination node receives the packet and delivers it to the destination Pod through its local CNI datapath.

<img width="1774" height="887" alt="ChatGPT Image Sep 15, 2026, 05_14_02 PM" src="https://github.com/user-attachments/assets/a5f91025-41c0-440f-bd37-1225846e8514" />

## 4.3 Pod to Service Communication

Interview answer

A Kubernetes Service provides a stable virtual IP and DNS name for a group of Pods selected by labels. When a client Pod sends traffic to the Service ClusterIP, the Service dataplane selects one of the healthy backend endpoints and forwards the traffic to that Pod. Traditionally this forwarding is implemented by kube-proxy using iptables or IPVS. In some environments, an eBPF-based CNI such as Cilium performs the Service load balancing instead.

<img width="1774" height="887" alt="image" src="https://github.com/user-attachments/assets/02587c44-0fc5-4f25-a2c0-b11eb60c0e97" />
...
CNI means **Container Network Interface**. It is a specification and set of conventions that allow a container runtime to ask a networking plugin to configure networking for a container or Pod network namespace.

> 🚩 **Note!**
>
> The normal Kubernetes flow is:
>
> ```text
> Kubelet
>     ↓
...
> ```
>
> The kubelet requests the container runtime to create the Pod sandbox. The runtime prepares the Pod network namespace and invokes the CNI plugin using the CNI `ADD` operation. The CNI plugin configures the Pod network. IP allocation is commonly delegated to an **IPAM plugin**.

## CNI responsibilities

Depending on the implementation, a CNI plugin may:

- Create or attach Pod network interfaces.
- Create a veth pair, attach an interface to a bridge, or attach an ENI.
- Request an IP address from an IPAM plugin.
- Configure routes, neighbors, and gateway information.
- Configure native routing or overlay/encapsulation.
- Install eBPF programs when the CNI uses an eBPF datapath.
- Enforce NetworkPolicy when policy enforcement is supported.
- Remove interfaces, routes, IP allocations, and other resources during CNI `DEL`.

> **Important:** CNI is the interface/specification. Calico, Cilium, Flannel, and AWS VPC CNI are examples of networking implementations.
...
**eBPF** is a Linux kernel technology that allows verified programs to run at supported kernel hooks. In networking, eBPF programs can inspect, filter, redirect, load-balance, secure, and observe packets.

## eBPF is a technology, not a Kubernetes networking model

```text
CNI implementation
       │
       ├── may use veth / bridge / routing
       ├── may use ENIs
       └── may use eBPF
```

A CNI such as Cilium can use eBPF for:

- Pod-to-Pod forwarding
- Service load balancing
- NetworkPolicy enforcement
- Network visibility and tracing
- kube-proxy replacement

> 🚩 **Note!**
>
> eBPF does not mean that traffic leaves Linux or bypasses the kernel. eBPF programs run inside the Linux kernel at supported hooks.

## eBPF versus iptables

| Feature | iptables | eBPF |
|---|---|---|
| Mechanism | Netfilter rules | Verified kernel programs |
| Service handling | Common traditional method | Can implement Service datapath |
| Scaling | Rule-chain processing | Maps and direct lookups may improve scaling |
| kube-proxy replacement | No | Possible with supported implementation |
| Observability | Requires additional tools | Can emit detailed flow events |

---

# 15. kube-proxy and Service networking

`kube-proxy` traditionally implements Kubernetes **Service** traffic. It watches Services and EndpointSlices and programs packet-processing rules so traffic sent to a Service virtual IP can reach a backend Pod endpoint.
...
Direct Pod-to-Pod traffic does not inherently require kube-proxy because it does not use a Service ClusterIP. The CNI and node networking provide the connectivity.
...
Traditional kube-proxy modes include:

- `iptables`
- `IPVS`

Modern networking implementations can provide Service handling with eBPF and may replace kube-proxy.

## Are CNI, kube-proxy, and eBPF all required?

**No. They are not three mandatory components that must always run together.**

| Component | Main job | Always required? |
|---|---|---|
| CNI / networking plugin | Configure Pod networking | A Pod networking implementation is required |
| kube-proxy | Traditional Service datapath | No; depends on Service implementation |
| eBPF | Kernel technology for networking/security/observability | No; only if the chosen implementation uses it |

### Common setup: traditional
...
eBPF datapath
    ...
    ├── Service load balancing
    ├── NetworkPolicy
    └── Optional kube-proxy replacement
```

> **Interview answer:** CNI configures Pod networking. kube-proxy traditionally implements Service forwarding. eBPF is a Linux kernel technology that a CNI such as Cilium may use for networking, Service load balancing, policy, and observability.
