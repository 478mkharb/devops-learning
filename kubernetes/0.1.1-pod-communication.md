...
>[!Note] 
>Kubelet requests the container runtime to create the Pod sandbox. The container runtime creates the network namespace and invokes the CNI plugin using the `CNI ADD` command. The CNI plugin then creates/configures the Pod’s network interface, assigns its IP address, and installs the required routes.
> `Kubelet → Container Runtime → CNI Plugin → Pod eth0`
