...
It is a specification and set of conventions used by container runtimes
and networking plugins to configure network connectivity for containers
and Pod network namespaces.

CNI defines how a container runtime invokes networking plugins to perform
operations such as:

- Creating or configuring network interfaces
- Connecting a Pod to the node network
- Assigning IP addresses through IPAM
- Configuring routes and neighbors
- Removing networking configuration when the Pod is deleted

> [Note!]
>
> The kubelet requests the container runtime to create the Pod sandbox.
> The container runtime creates or prepares the Pod network namespace
> and invokes the CNI plugin using the CNI `ADD` command.
>
> The CNI plugin then configures the Pod network interface, assigns an
> IP address through IPAM, and installs the required routes.
>
> ```text
> Kubelet
>     ↓
...
Depending on the CNI implementation, the CNI plugin may:

- Create or attach network interfaces.
- Connect the Pod to the node network.
- Request or allocate Pod IP addresses through an IPAM plugin.
- Configure Pod routes and neighbors.
- Configure Linux bridges, veth pairs, ENIs, or other datapath components.
- Configure tunnels or encapsulation where required.
- Install eBPF programs when supported by the CNI implementation.
- Enforce NetworkPolicy when supported.
- Remove networking configuration and release resources during CNI `DEL`.
