...
# Kubernetes DNS — Corrected Notes

## Why Kubernetes Needs DNS

Pod IPs are dynamic. A Pod can be deleted and recreated with a different IP.
...
Applications therefore use a stable **Service name** instead of directly using Pod IPs.
...
and is accessed through a Service commonly named:
...
CoreDNS  = DNS server implementation
```
...
Every normal Pod has a DNS resolver configuration file:
...
That address is commonly the ClusterIP of the Kubernetes DNS Service (`kube-dns`).

### `search`

The search domains allow an application to use a short name such as:
...
It mainly tells the Pod's DNS resolver:
...
So there are actually **two flows**:

### DNS flow

```text
Pod → kube-dns Service → CoreDNS → Service ClusterIP
```
...
The individual Pod IPs are **different and can change**.

---

# Where Does CoreDNS Get Service/Endpoint Information?
...
For Service discovery, **EndpointSlices** are especially important because they represent the endpoints associated with a Service.
...
CoreDNS watches Kubernetes information through the API and maintains the DNS information it needs. It does **not** depend on a static text file containing every Pod IP.
...
A Headless Service has **no virtual ClusterIP**.
...
The exact DNS response behavior depends on the Service/endpoints configuration, but the key point is that there is **no ClusterIP**.

---

# Why Use a Headless Service?

A Headless Service is useful when an application needs to discover **individual endpoints**, rather than accessing a single virtual Service IP.

It is commonly used with **StatefulSets** and distributed/stateful applications.
...
A Headless Service can provide DNS-based discovery for these Pods.

Conceptually, individual DNS names can look like:
...
From a Pod in the same namespace, the shorter name normally works:

```bash
curl http://backend
```

because of the DNS search domains configured in `/etc/resolv.conf`.
...
## 2. Check the Kubernetes DNS Service
...
```bash
kubectl exec -it <pod> -- getent hosts backend
```
...
| `10.96.120.45` | `backend` Service | Stable virtual IP for backend Service |
| `10.244.1.15` | backend Pod | IP of one individual Pod |
...
## Q2. Does `/etc/resolv.conf` contain all Service IPs?

> No. It tells the Pod where to send DNS queries; it does not contain all Kubernetes DNS records.

## Q3. Does every Pod behind a Service have the same IP?

> No. Each Pod has its own Pod IP. The Service has one stable virtual ClusterIP representing the Service.

## Q4. What is CoreDNS?

> CoreDNS is the Kubernetes DNS server used for cluster DNS and service discovery.

## Q5. Where does CoreDNS get Kubernetes DNS information?

> CoreDNS watches Kubernetes resources through the Kubernetes API, especially Services and EndpointSlices, and maintains the DNS information it needs.

## Q6. What is a Headless Service?

> A Service with `clusterIP: None`. It does not provide a virtual ClusterIP; DNS can return the Service's endpoint Pod IPs.

## Q7. Why use a Headless Service?

> It is commonly used for endpoint discovery and stable DNS identities with StatefulSets and distributed stateful applications.
...
Think of the four pieces like this:
...
CoreDNS
        ↓
"WHAT DNS answer should I return?"
...
### Multiple Pods Behind One Service
...
> **Pod IPs are dynamic, so applications use Service DNS names. `/etc/resolv.conf` tells the Pod where to send DNS queries, CoreDNS resolves Kubernetes DNS names using information from the Kubernetes API, a normal Service provides a stable ClusterIP in front of Pods, and a Headless Service uses `clusterIP: None` so DNS can expose individual endpoints instead of a virtual ClusterIP.**
